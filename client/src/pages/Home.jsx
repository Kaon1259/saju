...
function DailyTarot({ variant = 'hero' }) {
  ...
  if (variant === 'row') {
    // 가로 카드 — 작은 카드 + 우측 라벨/버튼
    return (
      <>
        <button className="home-tarot-row" onClick={handleClick} aria-label="오늘의 타로 한 장 뽑기">
          <div className={`home-tarot-row-card ${flipped ? 'flipped' : ''} ${!flipped ? 'glow' : ''}`}>
            <div className="home-tarot-row-face home-tarot-row-back">
              <img src={backSrc} alt="" draggable={false} onError={(e) => { e.target.style.display = 'none'; }} />
            </div>
            <div className="home-tarot-row-face home-tarot-row-front">
          ...
          <span className="home-tarot-row-arrow">›</span>
        </button>
        {modalOpen && (
          <DailyTarotModal
            card={card}
...
function DailyTarotModal({ card, frontSrc, onClose, autoStart = false }) {
  ...
  return createPortal((
    <div className="dt-detail-overlay fade-in" onClick={onClose} role="dialog" aria-modal="true">
      {/* 상단 고정 닫기 버튼 — 어디서든 한 번에 닫기 */}
      <button
        className="dt-detail-close"
        onClick={(e) => { e.stopPropagation(); onClose(); }}
        aria-label="닫기"
      >
        <span className="dt-detail-close-x">✕</span>
        <span className="dt-detail-close-label">닫기</span>
      </button>
      ...
      <div className="dt-detail">
        ...
        <button className="dt-detail-back" onClick={(e) => { e.stopPropagation(); onClose(); }}>
          ← 홈으로 돌아가기
        </button>
        <p className="dt-detail-dismiss" onClick={onClose}>또는 빈 공간을 탭하세요</p>
      </div>
    </div>
  ), document.body);
}
...
// ════════════════════════════════════════════════════════════════
// Co-Star 스타일 한 줄 시그니처 — 매일 시드로 회전
// ════════════════════════════════════════════════════════════════
const LOVE_HEADLINES = [
  '오늘 누군가가 당신을 떠올립니다.',
  '마음을 들켜도 괜찮은 날.',
  '연락이 늦어도 진심은 가까이 있어요.',
  '용기 한 번이 일주일을 바꿉니다.',
  '다정함이 가장 강한 무기인 하루.',
  '진심을 숨기지 마세요. 오늘은 그게 답.',
  '망설였던 한마디, 오늘이 적기예요.',
  '먼저 다가갈수록 운이 따라옵니다.',
  '작은 시그널을 놓치지 마세요.',
  '오늘 만난 사람이 길게 갈 수도.',
  '예전 인연이 다시 떠오를 수 있어요.',
  '익숙한 관계에 새 호흡을 불어넣어 보세요.',
  '솔직한 표현이 가장 큰 매력.',
  '거리를 두는 것도 사랑의 기술.',
];

function pickDailyHeadline() {
  const d = new Date();
  const seed = d.getFullYear() * 1000 + (d.getMonth() + 1) * 50 + d.getDate();
  return LOVE_HEADLINES[seed % LOVE_HEADLINES.length];
}
...
function reorderLoveBanners(status) {
  const order = (() => {
    switch (status) {
      case 'IN_RELATIONSHIP':
      case 'MARRIED':
        return ['mylove', 'rel', 'compat', 'confess', 'star', 'meet', 'some', 'crush'];
      case 'SOME':
        return ['some', 'confess', 'rel', 'mylove', 'compat', 'crush', 'star', 'meet'];
      case 'SINGLE':
        return ['rel', 'crush', 'meet', 'some', 'star', 'compat', 'mylove', 'confess'];
      case 'COMPLICATED':
        return ['rel', 'meet', 'mylove', 'compat', 'crush', 'star', 'some', 'confess'];
      default:
        return ['mylove', 'rel', 'compat', 'some', 'crush', 'confess', 'meet', 'star'];
    }
  })();
  return order.map(k => LOVE_BANNERS.find(b => b.id === k)).filter(Boolean);
}
...
function Home() {
  ...
  // 점신 + Co-Star 스타일 통합 변수
  const fallbackGrad = TIMEBAND_FALLBACK_GRAD[timeBand.id] || TIMEBAND_FALLBACK_GRAD.noon;
  const heroWeather = weatherData || { ...FALLBACK_WEATHER_BASE, ...fallbackGrad };
  ...
  const headline = useMemo(() => pickDailyHeadline(), []);
  const loveBtn = useMemo(() => {
    if (!userId) return { icon: '💕', label: '오늘의 연애운 보기', path: '/love-fortune' };
    switch (myData?.user?.relationshipStatus) {
      case 'IN_RELATIONSHIP':
      case 'MARRIED':
        return { icon: '💑', label: '나의 연인 보기', path: '/my-love-compat' };
      case 'SOME':
        return { icon: '💘', label: '썸·짝사랑 보기', path: '/my-some-crush' };
      case 'COMPLICATED':
        return { icon: '🌙', label: '다시 만날까 보기', path: '/again-meet' };
      case 'SINGLE':
        return { icon: '🙋', label: '솔로 운세 보기', path: '/my-solo' };
      default:
        return { icon: '💕', label: '오늘의 연애운 보기', path: '/love-fortune' };
    }
  }, [userId, myData?.user?.relationshipStatus]);
  const loveBanners = useMemo(() => reorderLoveBanners(myData?.user?.relationshipStatus), [myData?.user?.relationshipStatus]);

  return (
    <div className="home home-jeomshin">
      {/* ════════════════════════════════════════════════════════════ */}
      {/* 1. Hero — 오늘의 연애 (Co-Star 스타일 시그니처 카드)           */}
      {/* ════════════════════════════════════════════════════════════ */}
      <section
        className={`home-hero-cs home-hero-cs--${timeBand.id}`}
        style={{ '--w-from': heroWeather.bgFrom, '--w-to': heroWeather.bgTo }}
      >
        <div className="home-hero-cs-orb home-hero-cs-orb--1" />
        <div className="home-hero-cs-orb home-hero-cs-orb--2" />
        <div className="home-hero-cs-hearts" aria-hidden="true">
          {Array.from({ length: Math.max(10, Math.floor((loveScore || 50) / 5)) }).map((_, i) => {
            const total = Math.max(10, Math.floor((loveScore || 50) / 5));
            return (
              <span key={i} className="home-hero-cs-heart" style={{
                '--hf-x': `${5 + (i * 95 / total) % 92}%`,
                '--hf-delay': `${(i * 0.45) % 6}s`,
                '--hf-dur': `${3.5 + (i % 4) * 0.8}s`,
                '--hf-size': `${10 + (i % 5) * 4}px`,
              }}>&#x2764;</span>
            );
          })}
        ...
        <div className="home-hero-cs-meta">
          ...
          <button
            ...
            disabled={!userId}
          >
            {heroWeather.icon} {heroWeather.temp}° {heroWeather.conditionLabel || heroWeather.condition}
            {userId && <span className="home-hero-cs-meta-arrow">›</span>}
          </button>
          <span className="home-hero-cs-meta-sep">·</span>
          <span>{timeBand.icon} {timeBand.label}</span>
        </div>

        {/* Co-Star 시그니처 — 큰 한 줄 */}
        <div className="home-hero-cs-signature">
          <span className="home-hero-cs-greeting">
            {userId ? `${userName || '오늘'}님의 연애` : '오늘의 연애'}
          </span>
            ...
            >
              ...
              <div className="home-love-banner-text">
                <div className="home-love-banner-title-row">
                  ...
                  {b.badge && <span className="home-love-banner-badge">{b.badge}</span>}
                </div>
                <span className="home-love-banner-sub">{b.sub}</span>
              </div>
