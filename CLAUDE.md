═══════════════════════════════════════════════════════════════
[페르소나] 이 프로젝트에서 Claude가 취하는 관점 (2026-07-03 정의)
═══════════════════════════════════════════════════════════════
너는 두 정체성을 동시에 갖춘 시니어 파트너로서 이 프로젝트를 대한다.

## ① 사주 전문가 (30년)
- 명리학(사주팔자)을 30년간 연구·임상한 전문가. 일간·십성·오행 생극·용신·대운/세운·
  십이운성·신살을 실제로 계산해서 해석한다. "좋아요/나빠요" 두루뭉술한 점(占)이 아니라
  **근거(어느 오행이 어느 십성으로 작용) → 타이밍(언제가 유리/불리) → 행동 지침** 순으로 말한다.
- 앱의 AI 프롬프트·운세 카피를 볼 때 "명리적으로 말이 되는가"를 항상 검증한다.
  즉흥 창작·모순된 간명을 잡아낸다. 단정하되 "운세의 관점"임을 정직하게 밝힌다.

## ② 모바일/서버 앱 개발 전문가 (20년)
- 안드로이드 네이티브·하이브리드(현재 Capacitor+React), Spring 백엔드, 결제(IAP/구독),
  광고(AdMob), 인증(OAuth/JWT), 배포(Railway)까지 풀스택으로 20년 실전 경험.
- 코드 품질·성능·보안·출시 리스크를 동시에 본다. 화려함보다 **출시 가능성**을 우선한다.
  UI 폴리싱에 매몰되지 않고, 매출·리텐션에 직결되는 것부터 처리한다.

## ③ 현재 미션 (최종 목표 = 정식 서비스 출시)
- **안드로이드 사주 앱 "1:1연애"(com.love.onetoone)** 를 만들어 **Google Play 정식 출시**하는 것이
  최종 목표. 지금은 출시 직전 단계 + BM(하트 경제 수익화) 구현 중.
- 매 결정을 "이게 출시와 매출에 어떻게 기여하는가" 기준으로 판단한다. 명리 전문성으로 콘텐츠
  품질을, 개발 전문성으로 실행을 담보한다. 두 관점이 충돌하면 사용자에게 트레이드오프를 제시한다.

═══════════════════════════════════════════════════════════════
[전략 검토] 스토어 등록명 — ASO(검색 노출) 관점 (2026-05-26)
═══════════════════════════════════════════════════════════════
※ 출시 시 Play Console 등록명 결정용. 아직 미확정(이름 방향 택1 대기).

## 핵심 원리
- 검색에 잡히는 건 **브랜드명이 아니라 "제목에 박은 키워드"**.
  · "1:1연애"는 검색량 0(신규 앱 + 브랜드 인지도 없음) → 이 단어 자체는 노출 기여 X.
  · 노출은 그 뒤 키워드(사람들이 실제 타이핑하는 단어)가 만든다.
- **구글 플레이는 별도 키워드 필드 없음**(애플은 100자 키워드 필드 있음).
  → **제목(30자) + 짧은 설명(80자)**이 키워드를 넣을 거의 전부. 자세한 설명(4000자)은 약하게 반영.

## 노려야 할 고볼륨 키워드 (운세 앱 기준, 검색량 대략 순)
운세 > 사주 > 무료(운세/사주) > 궁합 > 타로 > 연애운 > 오늘의 운세 > 토정비결 > 별자리
→ `연애운·사주·궁합·타로·운세·무료` 6개를 제목에 다 넣는 게 노출 핵심.

## 위치 = 트레이드오프
- 제목 **첫 단어가 검색 가중치 최고**.
- 브랜드 우선(관례): `1:1연애 - 무료 운세 사주 궁합 타로 연애운` (노출 좋음)
- **키워드 우선(노출 최대)**: `무료 운세 사주 - 1:1연애 궁합 타로 연애운` (28자)
  · 브랜드 검색량이 0인 출시 초반엔 키워드를 앞에 두는 게 유리. 브랜드 인지 후 앞으로 복귀.
- 런처 아이콘 밑 짧은 이름은 "1:1연애" 유지(Play 등록 제목과 별개 필드).

## 짧은 설명(80자, 두 번째로 중요) 초안
"사주·타로·궁합으로 보는 1:1 연애운세. 재회·고백·결혼 결정 상담까지, 오늘의 운세를 무료로 확인하세요."

## ⚠️ 솔직한 한계 (제목만으로 안 됨)
- 제목 키워드는 필요조건이지 충분조건 X. "운세"처럼 경쟁 치열한 head 키워드는
  포스텔러·점신(수백만 DL)이 점령 → 신규 앱 상위 노출 어려움.
- 구글 플레이 랭킹은 **설치 속도·평점·리텐션·삭제율**이 더 크게 작용.
- 전략: ① 롱테일 키워드("연애운세"·"연애 사주"·"재회 궁합") 공략 ② 초반 설치·평점 빠르게 축적
  (앱 내 평점 후기 +50하트 보상이 이걸 돕는 장치).

## 미확정 — 다음 결정
- 키워드 우선 vs 브랜드 우선 택1 → 결정 시 등록 제목 + 짧은 설명 + 자세한 설명(4000자) 초안 작성.
...
───────────────────────────────────────────────────────────────
[BM 설계 — 하트 경제 수익화] (2026-05-24 착수 / 2026-05-25 숫자 전부 확정)
───────────────────────────────────────────────────────────────
※ ①~⑤ 숫자 **전부 확정 완료(2026-05-25)**. 이제 남은 건 ①②③ 결제·광고·구독 "구현".

## 확정된 결정 (2026-05-24~25)
...
## ✅ 확정 숫자 (2026-05-25)
④ 무료 적립 (박하게) — **코드 반영 완료**(HeartPointConfigInitializer + AttendanceService 상수
   + HeartPointService 폴백 + 클라 Attendance/Landing/Invite/RatingClaim 표기):
   - 가입 30 + 프로필 30 = 진성 60 / 출석 2/일 / 연속 7·14·30일 10·20·40 / 초대 ±20 / 평점 30
⑤ 소비 단가 — **이미 구현·유지**: 룰 1 / 짧은AI 3 / 기본·연애 5 / 궁합·기간 8 / 타로 10·25·50 / 심화 30 / 간판(심화프리미엄·결정상담) 80
① IAP 충전 (5단계, 1하트≈5원, Play 수수료 15% 가정 마진 건전):
   - ₩550→90💗(입문) / ₩1,100→200💗 / ₩3,300→700💗(+6%) / ₩5,500→1,300💗(+18%) / ₩11,000→2,800💗(+27%)
② 리워드 광고 : 1회 +5💗, **하루 5회(25💗)** 한도 — 무과금 리텐션(후하게)
③ 구독 : 월 ₩9,900 → **일반(≤5💗)만 무제한** + 매월 50💗(박하게) + 광고 제거.
   단 간판(궁합·기간·타로·심화·결정상담)은 구독에서도 하트 소비(매출 보존)

## ⚠️ 남은 구현 (미연동 — 매출 직결, 최우선)
- **① IAP 결제 경로 미구현** (Google Play Billing). 충전 패키지 UI/구매/서버검증(영수증)/지급 전무.
  · Play Console에 인앱상품 5종 등록(사용자 작업) 필요.
- **② AdMob Phase 0(시뮬레이션)** — adMob.js 실연동 안 됨. 리워드 광고 SDK + 하루 5회 한도 서버검증.
- **③ 구독** — Play 구독상품 + 엔타이틀먼트(구독상태) 서버 로직 + 일반 무제한 게이팅.
- 셋 다 붙기 전엔 하트를 살 방법이 없어 매출 0.

## 다음 세션 진입점
- 구현 순서: **① Play Billing(가장 중요) → ② AdMob 실연동 → ③ 구독**.
- 참고 메모리: project_bm_design / project_heart_system_v2 / project_admob_mediation / project_decision_consult / project_icon_autoconvert_and_leak_fix

═══════════════════════════════════════════════════════════════

[완료] 첫 시범 다운로드용 서명 AAB 빌드 — AdMob 테스트광고 그대로 (2026-05-26)
- 배경: Google Play 개발자 계정 발급 직후. BM ② AdMob을 "실연동"하기 전에, 먼저 내부 테스트
  트랙으로 시범 다운로드부터 하기로 함. AdMob Phase 1 코드(커밋 eb4b21e)는 이미 완성돼 있었음.
- ⚠️ 핵심 결정 — **시범 단계에선 테스트 광고 ID 그대로 유지**: 본인이 자기 앱의 실제 광고를
  클릭/시청하면 무효 트래픽으로 AdMob 계정이 영구 정지될 수 있음(구글이 가장 엄격히 단속).
  테스트 ID로도 "광고 보고 +5하트" 전체 흐름(노출→Rewarded 이벤트→하트 지급) 검증 가능, 위험 0.
  실제 AdMob ID 교체는 **정식 출시 직전**에 (adMob.js:21 REWARDED_AD_ID + AndroidManifest.xml:47
  APPLICATION_ID + adMob.js:19 USE_TEST_ADS=false). 검증만 하려면 본인 기기를 AdMob 테스트기기로 등록.
- 빌드 환경 확인: Node 22 / npm 10 OK. PATH java는 17이지만 gradle.properties의
  org.gradle.java.home이 Android Studio JBR(Java 21)을 가리켜 CLI 빌드 정상.
- 릴리즈 서명 신규 구축 (이번이 처음):
  · keystore 생성: client/android/app/release.keystore (alias love-app, RSA 2048, validity 10000)
    — JBR keytool로 비대화식 생성. SHA1 76:9C:D5:C8:5E:BB:43:6B:6E:FB:A1:E2:24:F8:E2:D5:92:36:D7:23
  · client/android/gradle.properties에 RELEASE_STORE_FILE/PASSWORD/KEY_ALIAS/KEY_PASSWORD 4줄 추가
    (사용자 지정 비번). gradle.properties·*.keystore·*.jks 모두 .gitignore로 제외 확인 — 자격증명 깃 미커밋.
- 빌드 흐름(검증됨): npm run build(9.2s) → npx cap sync android(admob 플러그인 등록 확인) →
  cd client/android && ./gradlew bundleRelease(3m34s, signReleaseBundle 실행) →
  **app/build/outputs/bundle/release/app-release.aab (~115MB)**. 용량 대부분 타로 덱 이미지(정식 전 다이어트 고려).
- ⚠️ 다음 업로드마다 client/android/app/build.gradle versionCode +1 필수(현재 2/1.0.1). Play가 동일 versionCode 거부.
- 사용자 작업(Play Console): 앱 만들기 → 테스트>내부 테스트>새 버전 → Play 앱 서명(기본) → AAB 업로드
  → 테스터 이메일 등록 → 옵트인 링크 다운로드. 앱 콘텐츠 선언 필수(개인정보처리방침 URL/데이터 보안/
  **광고 포함=예**/콘텐츠 등급/타겟 고객층). keystore 파일 반드시 백업(모든 업데이트 동일 키).
- 소스(서버/앱) 변경 없음 — 커밋·Railway 재배포 불필요. 참고 메모리: project_release_signing / project_admob_mediation.
- 시범 검증 후 다음 우선순위 = ① Play Billing(IAP 충전) 구현.

  ── Play Console "앱 콘텐츠" 답안 (이 앱 기준, 2026-05-26 확정) ──
  · 개인정보처리방침 URL: https://trustworthy-growth-production-5c9a.up.railway.app/privacy
    (Railway 프론트 라이브 확인 200. 앱 내 /privacy = client/public/legal/privacy-v1.md 동일내용, 시행일 2026-04-28.
     SPA 로딩 문제 시 원문 .../legal/privacy-v1.md 대안 제출 가능)
  · 데이터 보안: 수집=예 / 전송중 암호화=예(HTTPS) / 삭제요청 가능=예(회원탈퇴 즉시파기).
    수집유형 — 이름(닉네임)·사용자ID(카카오 회원번호)·기타 개인정보(생년월일/출생시간/성별/혈액형/MBTI/연인정보)·
    앱 활동(분석요청·하트사용 기록)·기기ID(OS/브라우저/IP). **제3자 공유는 전부 "아니요"**
    (Anthropic/Kakao/Railway는 위탁 처리자라 Google 기준 sharing 아님). ⚠️ 결제(구매내역)는 IAP 구현 후 추가 선언.
  · 콘텐츠 등급(IARC): 폭력/성적/욕설/마약/도박 전부 없음(하트=현금환전 불가 인앱재화→도박 아님), 디지털구매=현재 없음
    (IAP 붙이면 갱신) → 전체이용가 예상. 정직 기재 필수.
  · 타겟 고객층: **만 18세 이상만 체크**(연애·결제 앱), 만 13세 미만 절대 포함 금지(패밀리 정책 회피).
    "어린이 관심 유도? 아니요".
  · 광고 포함=예(AdMob). 앱 액세스 권한: 카카오 로그인 게이팅 있으나 **게스트 무료운세 모달로 로그인 없이 검토 가능** 명시
    → "일부 기능 로그인 필요" 선택 + 게스트 검토 안내(테스트 계정 불필요).

  ── ⚠️ 미결정(다음 세션) — 패키지 이름 충돌 (2026-05-26 발견) ──
  · AAB 업로드 시 Play Console 거부: "패키지 이름에 ymsoft.com.love 이 있어야 합니다".
  · 우리 앱 패키지 = **com.love.onetoone** (capacitor.config.json appId / build.gradle applicationId·namespace /
    AndroidManifest 딥링크 scheme). 'ymsoft.com.love'는 우리 코드에 전혀 없음 → Play Console 슬롯 쪽에서 온 것
    (그 앱 항목에 과거 ymsoft.com.love 업로드 이력이 있다는 뜻. 잘못된 앱 항목 가능성).
  · ⚠️ 패키지명은 출시 후 영구 변경 불가.
  · **A안(권장·코드 0)**: Play Console에서 새 앱 생성 → 이 AAB 업로드 → 첫 업로드가 com.love.onetoone 확정.
    ymsoft.com.love 슬롯 미사용.
  · **B안**: 패키지를 ymsoft.com.love로 변경 — capacitor/gradle/manifest + **카카오 OAuth 딥링크 스킴
    (com.love.onetoone://) 6곳**(AndroidManifest·MainActivity.java·Register.jsx·KakaoAuthController 등) 수정 +
    카카오 콘솔 스킴 재등록 + 재빌드 필요. ymsoft가 개발사 브랜드라 꼭 써야 할 때만.
  · → 사용자가 A/B 결정 후 진행.

  ── 진행 현황 + 다음 세션 계획 (2026-05-26 종료 시점) ──
  · ✅ 내부 테스트(Internal testing) 등록 성공 — 이메일 공유 → 옵트인 링크 → 다운로드 페이지 정상 노출 확인.
    (내부 테스트는 개인정보처리방침·데이터보안·콘텐츠등급·타겟 등 "앱 콘텐츠" 선언 없이도 진행됨 — 정상. 그 선언들은
     비공개/공개/프로덕션 승격 시 필수가 됨 → 위 답안 그대로 사용.)
  · ⏭️ **내일(2026-05-27)부터 비공개 테스트(Closed testing) 진입 예정.**
    이유: 신규 개인 개발자 계정(2023-11-13 이후)은 **비공개 테스트 테스터 12명 이상 / 14일 연속** 충족해야
    프로덕션 출시 자격이 열림(내부 테스트는 이 요건에 미포함). 14일 걸리니 빨리 시작. ※ 조직 계정이면 이 요건 면제.
  · ✅ 실기기 검증 완료(2026-05-26): 내부 테스트 트랙에서 다운로드(기존 dev 빌드 위에 "업데이트"로 교체, 116MB) →
    실행 → AdMob 테스트 광고 노출까지 확인. 패키지=com.love.onetoone로 정상 등록(ymsoft.com.love 충돌은 새 앱으로 해결).
    AdMob ② 시범 검증 성공 = 오늘 목표 달성. **하트 +5 지급까지 확인됨**(광고→서버 /api/hearts/ad-reward→잔액갱신
    전체 루프 동작). 하루 5회 한도 세부 동작만 추후 확인 권장.
