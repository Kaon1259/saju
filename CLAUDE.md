[완료] 동화 홈 신설 — 세 번째 홈 스타일 (동화책 컨셉 / 파스텔 핀·피치) (2026-05-17)
- new.png(인스타 SNS 화면) 참고 요청 → 사용자 의도는 "파스텔 톤 + 동화 같은 분위기". SNS 레이아웃 아님
- HomeStory.jsx/css 신규 — 감성 홈(Home)/심플 홈(HomeNew)에 이은 세 번째 홈. homeStyle='story'
  - 구성: 챕터 표지 인사말("오늘의 이야기") + 삽화풍 히어로(밤하늘 sky + 달/구름/별 + 점수 오브) + 오늘의 다섯 빛(5항목 별점) + 오늘 함께하는 행운(3버블) + 퀵(4) + 출석카드 + 챕터 3개(사랑/계절/운명 이야기)
  - 점수: HomeNew와 동일하게 saju.score(aiAnalyzed) 우선, 없으면 유저ID+날짜 시드 결정적 산출(hashFloat)
  - 다섯 빛: 연애/재물/인간관계/건강/직장 → value를 5단계 별점(★)으로. 챕터별 틴트(rose/peach/lilac)
  - 전부 CSS로 동화 분위기 구현(삽화 자산 없음): 파스텔 sky 그라데이션, 빛나는 오브/달, 드리프트 구름, 트윙클 별, 종이 카드, 점선 룰
- App.jsx HomeRouter: homeStyle 'new'→HomeNew / 'story'→HomeStory / 그 외→Home 3분기
- ChooseHome: 3번째 옵션 카드(동화 홈) + PreviewStory 미리보기 추가, 안내문 "두 가지"→"세 가지", 태그 정리(감성=기존/심플=SIMPLE/동화=NEW)
- Settings: 홈 화면 스타일 값 표기에 '동화 홈' 추가
- 라이트=피치~핑크 크림 배경 + 종이 카드 / 다크=트와일라잇 퍼플. 스키마/서버 변경 없음 — 클라 전용
- 앱 반영은 npm run build → npx cap sync 필요. new.png/gpt추천.png(참고 스크린샷)은 커밋 제외(직전 커밋 선례 따름)
